...
void countMotif (event e, set<pair<event, key>>& pre, map<event, map<string, int>>& motif_count, int N_vtx, int N_event, int d_c, bool is_target){
    vertex u = e.second.first;
    vertex v = e.second.second;
    timestamp t = e.first;
    vector<pair<event, key>> new_motif;    //used to store the new motifs
    for (auto it = pre.begin(); it != pre.end();) {   //for each current prefix
        event core = it->first;
        vector<event> KEY = it->second;
        set<vertex> nodes = getNodes(KEY);
        if (abs(t - core.first) <= d_c ) {  //check delta C and delta W
            if (nodes.find(u)!=nodes.end() || nodes.find(v)!=nodes.end()) {
                nodes.insert(u);
                nodes.insert(v);
                if (nodes.size() <= N_vtx) {    //check the number of vertices
                    pair<event, key> motif = make_pair(core, KEY);
//                    pair<event, key> motif1 = make_pair(e, KEY);
                    motif.second.push_back(e);
//                    motif1.second.push_back(e);
                    if (motif.second.size()<=N_event && nodes.size()<=N_vtx) {
                        string code = encodeMotif(motif.second);
                        motif_count[core][code] += 1;
//                        motif_count[e][code] += 1;
                    }
                    if(motif.second.size()<N_event) {
                        new_motif.push_back(motif);
//                        new_motif.push_back(motif1);
                    }
                }
            }
            ++it;
    ...
    //add the new motifs to the current prefix list
    if (!new_motif.empty()) {
        for (pair<event, key> const &mt: new_motif) {
            pre.insert(mt);
        }
    }
    if (is_target) {
        vector<event> E;
        E.push_back(e);
        pre.insert(make_pair(e, E)); // add the new event to the current prefix list
    }
    return;
}
